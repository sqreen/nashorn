...
import static io.sqreen.internal.org.objectweb.asm.Opcodes.H_INVOKEVIRTUAL;

import java.io.BufferedInputStream;
import java.io.FileInputStream;
import java.io.IOException;
import java.util.List;
import io.sqreen.internal.org.objectweb.asm.ClassReader;
...
import io.sqreen.nashorn.internal.tools.nasgen.MemberInfo.Kind;
...
public class ClassGenerator {
    ...
    void emitGetClassName(final String name) {
        final MethodGenerator mi = makeMethod(ACC_PUBLIC, StringConstants.GET_CLASS_NAME, StringConstants.GET_CLASS_NAME_DESC);
        mi.loadLiteral(name);
        mi.returnValue();
        mi.computeMaxs();
        mi.visitEnd();
    ...
    static MethodGenerator makeStaticInitializer(final ClassVisitor cv) {
        return makeStaticInitializer(cv, StringConstants.CLINIT);
    }
    ...
    static MethodGenerator makeStaticInitializer(final ClassVisitor cv, final String name) {
        ...
        final String desc = StringConstants.DEFAULT_INIT_DESC;
    ...
    static MethodGenerator makeConstructor(final ClassVisitor cv) {
        final int access = 0;
        final String name = StringConstants.INIT;
        final String desc = StringConstants.DEFAULT_INIT_DESC;
        final MethodVisitor mv = cv.visitMethod(access, name, desc, null, null);
        return new MethodGenerator(mv, access, name, desc);
    }
    ...
    static void emitStaticInitPrefix(final MethodGenerator mi, final String className, final int memberCount) {
        ...
        if (memberCount > 0) {
            // new ArrayList(int)
            mi.newObject(StringConstants.ARRAYLIST_TYPE);
            mi.dup();
            mi.push(memberCount);
            mi.invokeSpecial(StringConstants.ARRAYLIST_TYPE, StringConstants.INIT, StringConstants.ARRAYLIST_INIT_DESC);
            // stack: ArrayList
        } else {
            // java.util.Collections.EMPTY_LIST
            mi.getStatic(StringConstants.COLLECTIONS_TYPE, StringConstants.COLLECTIONS_EMPTY_LIST, StringConstants.LIST_DESC);
            // stack List
        }
    }
    ...
    static void emitStaticInitSuffix(final MethodGenerator mi, final String className) {
        ...
        mi.invokeStatic(StringConstants.PROPERTYMAP_TYPE, StringConstants.PROPERTYMAP_NEWMAP, StringConstants.PROPERTYMAP_NEWMAP_DESC);
        // $nasgenmap$ = pmap;
        mi.putStatic(className, StringConstants.PROPERTYMAP_FIELD_NAME, StringConstants.PROPERTYMAP_DESC);
        mi.returnVoid();
        mi.computeMaxs();
        mi.visitEnd();
    }
    ...
    private static Type memInfoType(final MemberInfo memInfo) {
        switch (memInfo.getJavaDesc().charAt(0)) {
            ...
            case 'L': return StringConstants.TYPE_OBJECT;
        }
    }
    ...
    static void addGetter(final ClassVisitor cv, final String owner, final MemberInfo memInfo) {
        ...
        final String name = StringConstants.GETTER_PREFIX + memInfo.getJavaName();
    ...
    static void addSetter(final ClassVisitor cv, final String owner, final MemberInfo memInfo) {
        final int access = ACC_PUBLIC;
        final String name = StringConstants.SETTER_PREFIX + memInfo.getJavaName();
    ...
    static void addMapField(final ClassVisitor cv) {
        ...
        final FieldVisitor fv = cv.visitField(ACC_PRIVATE | ACC_STATIC | ACC_FINAL,
            StringConstants.PROPERTYMAP_FIELD_NAME, StringConstants.PROPERTYMAP_DESC, null, null);
        if (fv != null) {
            fv.visitEnd();
        }
    }
    ...
    static void addFunctionField(final ClassVisitor cv, final String name) {
        addField(cv, name, StringConstants.OBJECT_DESC);
    }
    ...
    static void newFunction(final MethodGenerator mi, final String objName, final String className, final MemberInfo memInfo, final List<MemberInfo> specs) {
        ...
        if (!specs.isEmpty()) {
            ...
            mi.invokeStatic(StringConstants.SCRIPTFUNCTION_TYPE, StringConstants.SCRIPTFUNCTION_CREATEBUILTIN, StringConstants.SCRIPTFUNCTION_CREATEBUILTIN_SPECS_DESC);
        } else {
            mi.invokeStatic(StringConstants.SCRIPTFUNCTION_TYPE, StringConstants.SCRIPTFUNCTION_CREATEBUILTIN, StringConstants.SCRIPTFUNCTION_CREATEBUILTIN_DESC);
        }

        if (arityFound) {
            mi.dup();
            mi.push(memInfo.getArity());
            mi.invokeVirtual(StringConstants.SCRIPTFUNCTION_TYPE, StringConstants.SCRIPTFUNCTION_SETARITY, StringConstants.SCRIPTFUNCTION_SETARITY_DESC);
        }

        mi.dup();
        mi.loadLiteral(memInfo.getDocumentationKey(objName));
        mi.invokeVirtual(StringConstants.SCRIPTFUNCTION_TYPE, StringConstants.SCRIPTFUNCTION_SETDOCUMENTATIONKEY, StringConstants.SCRIPTFUNCTION_SETDOCUMENTATIONKEY_DESC);
    }
    ...
    static void linkerAddGetterSetter(final MethodGenerator mi, final String className, final MemberInfo memInfo) {
        ...
        String javaName = StringConstants.GETTER_PREFIX + memInfo.getJavaName();
        mi.visitLdcInsn(new Handle(H_INVOKEVIRTUAL, className, javaName, getterDesc(memInfo), false));
        // setup setter method handle
        if (memInfo.isFinal()) {
            mi.pushNull();
        } else {
            javaName = StringConstants.SETTER_PREFIX + memInfo.getJavaName();
        ...
        mi.invokeStatic(StringConstants.ACCESSORPROPERTY_TYPE, StringConstants.ACCESSORPROPERTY_CREATE, StringConstants.ACCESSORPROPERTY_CREATE_DESC);
        // boolean Collection.add(property)
        mi.invokeInterface(StringConstants.COLLECTION_TYPE, StringConstants.COLLECTION_ADD, StringConstants.COLLECTION_ADD_DESC);
    ...
    private static void loadFunctionName(final MethodGenerator mi, final String propertyName) {
        if (propertyName.startsWith(StringConstants.SYMBOL_PREFIX)) {
    ...
    private static void loadPropertyKey(final MethodGenerator mi, final String propertyName) {
        if (propertyName.startsWith(StringConstants.SYMBOL_PREFIX)) {
            mi.getStatic(StringConstants.NATIVESYMBOL_TYPE, propertyName.substring(2), StringConstants.SYMBOL_DESC);
        } else {
            mi.loadLiteral(propertyName);
        }
    }
