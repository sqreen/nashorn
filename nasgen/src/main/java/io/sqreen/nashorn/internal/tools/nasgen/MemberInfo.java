...
public final class MemberInfo implements Cloneable {
        ...
        switch (kind) {
            case CONSTRUCTOR: {
                ...
                if (argTypes.length > 2) {
                    ...
                    final boolean isVarArg = lastArgTypeDesc.equals(StringConstants.OBJECT_ARRAY_DESC);
                    if (!lastArgTypeDesc.equals(StringConstants.OBJECT_DESC) && !isVarArg) {
    ...
    public static boolean isScriptObject(final String name) {
        ...
        if (name.startsWith(StringConstants.OBJ_PKG + "Native") ||
            name.equals(StringConstants.OBJ_PKG + "Global") ||
            name.equals(StringConstants.OBJ_PKG + "ArrayBufferView")) {
            return true;
        }

        if (name.startsWith(StringConstants.RUNTIME_PKG)) {
        ...
        if (name.startsWith(StringConstants.SCRIPTS_PKG)) {
            final String simpleName = name.substring(name.lastIndexOf('/') + 1);
            switch (simpleName) {
                case "JD":
                case "JO":
    ...
    private static boolean isJSPrimitiveType(final Type type) {
        switch (type.getSort()) {
            ...
            case Type.DOUBLE:
                return true;
            default:
                return type != StringConstants.TYPE_SYMBOL;
        }
    }
    ...
    private static boolean isJavaLangObject(final Type type) {
        return type.getDescriptor().equals(StringConstants.OBJECT_DESC);
    }

    private static boolean isJavaLangString(final Type type) {
        return type.getDescriptor().equals(StringConstants.STRING_DESC);
    }
