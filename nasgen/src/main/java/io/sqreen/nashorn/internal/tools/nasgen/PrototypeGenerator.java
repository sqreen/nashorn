...
public class PrototypeGenerator extends ClassGenerator {
    ...
    byte[] getClassBytes() {
        ...
        cw.visit(V1_7, ACC_FINAL | ACC_SUPER, className, null, StringConstants.PROTOTYPEOBJECT_TYPE, null);
        if (memberCount > 0) {
            // add fields
            emitFields();
            // add <clinit>
    ...
    private void emitConstructor() {
        ...
        if (memberCount > 0) {
            ...
            mi.invokeSpecial(StringConstants.PROTOTYPEOBJECT_TYPE, StringConstants.INIT, StringConstants.SCRIPTOBJECT_INIT_DESC);
            // initialize Function type fields
            initFunctionFields(mi);
        } else {
            // call "super()"
            mi.invokeSpecial(StringConstants.PROTOTYPEOBJECT_TYPE, StringConstants.INIT, StringConstants.DEFAULT_INIT_DESC);
        }
        mi.returnVoid();
        mi.computeMaxs();
        mi.visitEnd();
    ...
    public static void main(final String[] args) throws IOException {
        ...
        try (FileOutputStream fos = new FileOutputStream(className + StringConstants.PROTOTYPE_SUFFIX + ".class")) {
