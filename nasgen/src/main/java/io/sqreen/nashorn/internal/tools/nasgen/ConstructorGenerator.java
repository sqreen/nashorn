...
public class ConstructorGenerator extends ClassGenerator {
    ...
    byte[] getClassBytes() {
        ...
        final String superClass = (constructor != null)? StringConstants.SCRIPTFUNCTION_TYPE : StringConstants.SCRIPTOBJECT_TYPE;
    ...
    private void emitFields() {
        ...
        for (MemberInfo memInfo : scriptClassInfo.getMembers()) {
            if (memInfo.isConstructorFunction()) {
                ...
                memInfo.setJavaDesc(StringConstants.OBJECT_DESC);
    ...
    private void emitConstructor() {
        ...
        if (constructor != null) {
            ...
            if (arity != MemberInfo.DEFAULT_ARITY) {
                mi.loadThis();
                mi.push(arity);
                mi.invokeVirtual(StringConstants.SCRIPTFUNCTION_TYPE, StringConstants.SCRIPTFUNCTION_SETARITY,
                        StringConstants.SCRIPTFUNCTION_SETARITY_DESC);
            }

            mi.loadThis();
            mi.loadLiteral(scriptClassInfo.getName());
            mi.invokeVirtual(StringConstants.SCRIPTFUNCTION_TYPE, StringConstants.SCRIPTFUNCTION_SETDOCUMENTATIONKEY,
                        StringConstants.SCRIPTFUNCTION_SETDOCUMENTATIONKEY_DESC);
        }
        mi.returnVoid();
        mi.computeMaxs();
        mi.visitEnd();
    ...
    private void loadMap(final MethodGenerator mi) {
        if (memberCount > 0) {
            mi.getStatic(className, StringConstants.PROPERTYMAP_FIELD_NAME, StringConstants.PROPERTYMAP_DESC);
        }
    }
    ...
    private void callSuper(final MethodGenerator mi) {
        ...
        if (constructor == null) {
            ...
            superClass = StringConstants.SCRIPTOBJECT_TYPE;
            superDesc = (memberCount > 0) ? StringConstants.SCRIPTOBJECT_INIT_DESC : StringConstants.DEFAULT_INIT_DESC;
            loadMap(mi);
        } else {
            // call Function.<init>
            superClass = StringConstants.SCRIPTFUNCTION_TYPE;
            superDesc = (memberCount > 0) ? StringConstants.SCRIPTFUNCTION_INIT_DESC4 : StringConstants.SCRIPTFUNCTION_INIT_DESC3;
        ...
        mi.invokeSpecial(superClass, StringConstants.INIT, superDesc);
    }
    ...
    private void initFunctionFields(final MethodGenerator mi) {
        ...
        for (final MemberInfo memInfo : scriptClassInfo.getMembers()) {
            ...
            mi.putField(className, memInfo.getJavaName(), StringConstants.OBJECT_DESC);
        }
    }
    ...
    private void initDataFields(final MethodGenerator mi) {
        ...
        for (final MemberInfo memInfo : scriptClassInfo.getMembers()) {
           ...
           if (value != null) {
               ...
               mi.newObject(clazz);
               mi.dup();
               mi.invokeSpecial(clazz, StringConstants.INIT, StringConstants.DEFAULT_INIT_DESC);
               mi.putField(className, memInfo.getJavaName(), memInfo.getJavaDesc());
           }
        }
    }
    ...
    private void initPrototype(final MethodGenerator mi) {
        ...
        mi.dup();
        mi.invokeSpecial(protoName, StringConstants.INIT, StringConstants.DEFAULT_INIT_DESC);
        mi.dup();
        mi.loadThis();
        mi.invokeStatic(StringConstants.PROTOTYPEOBJECT_TYPE, StringConstants.PROTOTYPEOBJECT_SETCONSTRUCTOR,
                StringConstants.PROTOTYPEOBJECT_SETCONSTRUCTOR_DESC);
        mi.invokeVirtual(StringConstants.SCRIPTFUNCTION_TYPE, StringConstants.SCRIPTFUNCTION_SETPROTOTYPE, StringConstants.SCRIPTFUNCTION_SETPROTOTYPE_DESC);
    }
    ...
    public static void main(final String[] args) throws IOException {
        ...
        try (FileOutputStream fos = new FileOutputStream(className + StringConstants.CONSTRUCTOR_SUFFIX + ".class")) {
