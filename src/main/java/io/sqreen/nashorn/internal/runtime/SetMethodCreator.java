...
final class SetMethodCreator {
    ...
    private SetMethod createExistingPropertySetter() {
        ...
        if (NashornCallSiteDescriptor.isDeclaration(desc) && property.needsDeclaration()) {
            ...
            final MethodHandle slowSetter = Lookup.MH.insertArguments(ScriptObject.DECLARE_AND_SET, 1, getName()).asType(fastSetter.type());
            ...
            casMap = Lookup.MH.dropArguments(casMap, 1, type);
    ...
    private SetMethod createGlobalPropertySetter() {
        ...
        return new SetMethod(Lookup.MH.filterArguments(global.addSpill(type, getName()), 0, ScriptObject.GLOBALFILTER), null);
    }
    ...
    private SetMethod createNewSetter(final Property property, final SwitchPoint builtinSwitchPoint) {
        ...
        MethodHandle slowSetter = ScriptObject.SET_SLOW[JSType.getAccessorTypeIndex(type)];
        slowSetter = Lookup.MH.insertArguments(slowSetter, 3, NashornCallSiteDescriptor.getFlags(desc));
        ...
        extCheck = Lookup.MH.dropArguments(extCheck, 1, type);

        MethodHandle nop = JSType.VOID_RETURN.methodHandle();
        nop = Lookup.MH.dropArguments(nop, 0, Object.class, type);
