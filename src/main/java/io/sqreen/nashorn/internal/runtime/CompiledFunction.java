...
import io.sqreen.nashorn.internal.codegen.TypeMap;
...
import io.sqreen.nashorn.internal.runtime.events.RecompilationEvent;
...
final class CompiledFunction {
    ...
    CompiledFunction(final MethodHandle invoker, final MethodHandle constructor, final int flags, final MethodType callSiteType, final Specialization specialization, final DebugLogger log) {
        ...
        if (specialization != null && specialization.isOptimistic()) {
             ...
             */
            this.invoker = Lookup.MH.insertArguments(invoker, invoker.type().parameterCount() - 1, UnwarrantedOptimismException.FIRST_PROGRAM_POINT);
    ...
    private MethodHandle createInvokerForPessimisticCaller() {
        return createInvoker(Object.class, UnwarrantedOptimismException.INVALID_PROGRAM_POINT);
    }
    ...
    private static MethodHandle createConstructorFromInvoker(final MethodHandle invoker) {
        ...
        final MethodHandle filtered = Lookup.MH.foldArguments(Lookup.MH.dropArguments(NEWFILTER, 2, ctorArgs), returnsObject);
    ...
    private MethodHandle createRewriteExceptionHandler() {
        return Lookup.MH.foldArguments(RESTOF_INVOKER, Lookup.MH.insertArguments(HANDLE_REWRITE_EXCEPTION, 0, this, optimismInfo));
    }
    ...
    private synchronized MethodHandle handleRewriteException(final OptimismInfo oldOptInfo, final RewriteException re) {
        ...
        final MethodHandle restOf = restOfHandle(effectiveOptInfo, compiler.compile(fn, Compiler.CompilationPhases.GENERATE_BYTECODE_AND_INSTALL_RESTOF), canBeDeoptimized);
        ...
        final MethodHandle restOf =
                changeReturnType(
                        info.data.lookupCodeMethod(
                                ...
                                Lookup.MH.type(restOfFunction.getReturnType().getTypeClass(),
