...
import io.sqreen.nashorn.internal.lookup.MethodHandleFactory;
...
public class AccessorProperty extends Property {
    ...
    private static class Accessors {
        ...
        Accessors(final Class<?> structure) {
            ...
            for (int i = 0; i < fieldCount; i++) {
                ...
                objectGetters[i] = Lookup.MH.asType(Lookup.MH.getter(LOOKUP, structure, fieldName, typeClass), Lookup.GET_OBJECT_TYPE);
                objectSetters[i] = Lookup.MH.asType(Lookup.MH.setter(LOOKUP, structure, fieldName, typeClass), Lookup.SET_OBJECT_TYPE);
            }
            ...
            if (!StructureLoader.isSingleFieldStructure(structure.getName())) {
                for (int i = 0; i < fieldCount; i++) {
                    ...
                    primitiveGetters[i] = Lookup.MH.asType(Lookup.MH.getter(LOOKUP, structure, fieldNamePrimitive, typeClass), Lookup.GET_PRIMITIVE_TYPE);
                    primitiveSetters[i] = Lookup.MH.asType(Lookup.MH.setter(LOOKUP, structure, fieldNamePrimitive, typeClass), Lookup.SET_PRIMITIVE_TYPE);
                }
            }
        }
    }
    ...
    private AccessorProperty(final Object key, final int flags, final int slot, final MethodHandle getter, final MethodHandle setter) {
        ...
        if (getterType == int.class) {
            primitiveGetter = Lookup.MH.asType(getter, Lookup.GET_PRIMITIVE_TYPE);
            primitiveSetter = setter == null ? null : Lookup.MH.asType(setter, Lookup.SET_PRIMITIVE_TYPE);
        } else if (getterType == double.class) {
            primitiveGetter = Lookup.MH.asType(Lookup.MH.filterReturnValue(getter, ObjectClassGenerator.PACK_DOUBLE), Lookup.GET_PRIMITIVE_TYPE);
            primitiveSetter = setter == null ? null : Lookup.MH.asType(Lookup.MH.filterArguments(setter, 1, ObjectClassGenerator.UNPACK_DOUBLE), Lookup.SET_PRIMITIVE_TYPE);
        } else {
        ...
        objectGetter  = getter.type() != Lookup.GET_OBJECT_TYPE ? Lookup.MH.asType(getter, Lookup.GET_OBJECT_TYPE) : getter;
        objectSetter  = setter != null && setter.type() != Lookup.SET_OBJECT_TYPE ? Lookup.MH.asType(setter, Lookup.SET_OBJECT_TYPE) : setter;
    ...
    private void initGetterSetter(final Class<?> structure) {
        ...
        if (isParameter() && hasArguments()) {
            ...
            final MethodHandle arguments = Lookup.MH.getter(LOOKUP, structure, "arguments", ScriptObject.class);
            objectGetter = Lookup.MH.asType(Lookup.MH.insertArguments(Lookup.MH.filterArguments(ScriptObject.GET_ARGUMENT.methodHandle(), 0, arguments), 1, slot), Lookup.GET_OBJECT_TYPE);
            objectSetter = Lookup.MH.asType(Lookup.MH.insertArguments(Lookup.MH.filterArguments(ScriptObject.SET_ARGUMENT.methodHandle(), 0, arguments), 1, slot), Lookup.SET_OBJECT_TYPE);
    ...
    private MethodHandle generateSetter(final Class<?> forType, final Class<?> type) {
        return debug(ObjectClassGenerator.createSetter(forType, type, primitiveSetter, objectSetter), getLocalType(), type, "set");
    }
    ...
    public MethodHandle getSetter(final Class<?> type, final PropertyMap currentMap) {
        ...
        final int currentTypeIndex = JSType.getAccessorTypeIndex(getLocalType());
        ...
        if (isBuiltin()) {
           mh = Lookup.MH.filterArguments(mh, 0, debugInvalidate(Lookup.MH.insertArguments(INVALIDATE_SP, 0, this), getKey().toString()));
        }
    ...
    private MethodHandle debug(final MethodHandle mh, final Class<?> forType, final Class<?> type, final String tag) {
        ...
        return context.addLoggingToHandle(
                ...
                new Supplier<String>() {
                    @Override
                    public String get() {
                        return tag + " '" + getKey() + "' (property="+ Debug.id(this) + ", slot=" + getSlot() + " " + getClass().getSimpleName() + " forType=" + MethodHandleFactory.stripName(forType) + ", type=" + MethodHandleFactory.stripName(type) + ')';
                    }
                });
    }
