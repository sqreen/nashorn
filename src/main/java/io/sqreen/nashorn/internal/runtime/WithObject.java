...
import io.sqreen.nashorn.api.scripting.AbstractJSObject;
...
public final class WithObject extends Scope {
    ...
    public GuardedInvocation lookup(final CallSiteDescriptor desc, final LinkRequest request) {
        ...
        if (fallBack != null) {
            ...
            if (find != null) {
                if (ScriptObject.NO_SUCH_METHOD_NAME.equals(fallBack)) {
                    link = expression.noSuchMethod(desc, request).addSwitchPoint(getProtoSwitchPoint(name));
                } else if (ScriptObject.NO_SUCH_PROPERTY_NAME.equals(fallBack)) {
    ...
    protected Object invokeNoSuchProperty(final Object key, final boolean isScope, final int programPoint) {
        final FindProperty find = expression.findProperty(ScriptObject.NO_SUCH_PROPERTY_NAME, true);
        if (find != null) {
            final Object func = find.getObjectValue();
            if (func instanceof ScriptFunction) {
                final ScriptFunction sfunc = (ScriptFunction)func;
                final Object self = isScope && sfunc.isStrict()? ScriptRuntime.UNDEFINED : expression;
    ...
    private static MethodHandle filterGuardReceiver(final GuardedInvocation link, final MethodHandle receiverFilter) {
        ...
        final MethodHandle filter = Lookup.MH.asType(receiverFilter,
