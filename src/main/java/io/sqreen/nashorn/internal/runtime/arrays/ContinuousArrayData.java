...
package io.sqreen.nashorn.internal.runtime.arrays;
    ...
    public GuardedInvocation findFastGetIndexMethod(final Class<? extends ArrayData> clazz, final CallSiteDescriptor desc, final LinkRequest request) {
        ...
        if (ContinuousArrayData.class.isAssignableFrom(clazz) && indexType == int.class) {
            ...
            if (has(index)) {
                ...
                final int          programPoint = NashornCallSiteDescriptor.isOptimistic(desc) ? NashornCallSiteDescriptor.getProgramPoint(desc) : UnwarrantedOptimismException.INVALID_PROGRAM_POINT;
                ...
                if (getElement != null) {
                    ...
                    final MethodHandle guard = Lookup.MH.insertArguments(FAST_ACCESS_GUARD, 0, clazz);
