...
public abstract class Property implements Serializable {
    ...
    static int mergeFlags(final PropertyDescriptor oldDesc, final PropertyDescriptor newDesc) {
        ...
        value = newDesc.has(PropertyDescriptor.CONFIGURABLE) ? newDesc.isConfigurable() : oldDesc.isConfigurable();
        if (!value) {
            propFlags |= NOT_CONFIGURABLE;
        }

        value = newDesc.has(PropertyDescriptor.ENUMERABLE) ? newDesc.isEnumerable() : oldDesc.isEnumerable();
        if (!value) {
            propFlags |= NOT_ENUMERABLE;
        }

        value = newDesc.has(PropertyDescriptor.WRITABLE) ? newDesc.isWritable() : oldDesc.isWritable();
        if (!value) {
