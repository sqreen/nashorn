...
public class Lexer extends Scanner {
    ...
    protected void add(final TokenType type, final int start, final int end) {
        ...
        if (type == TokenType.EOL) {
            ...
            if (pendingLine != -1) {
                stream.put(Token.toDesc(TokenType.EOL, linePosition, pendingLine));
                pendingLine = -1;
            }
    ...
    private void skipEOL(final boolean addEOL) {
        ...
        if (addEOL) {
            // Add an EOL token.
            add(TokenType.EOL, position, line);
        }
    }
    ...
    private void scanIdentifierOrKeyword() {
        ...
        if (type == TokenType.FUNCTION && pauseOnFunctionBody) {
    ...
    private String valueOfString(final int start, final int length, final boolean strict) throws RuntimeException {
        ...
        while (position < end) {
            ...
            if (ch0 == '\\') {
                ...
                case '7': {
                    if (strict) {
                        ...
                        if (next != '0' || (ch0 >= '0' && ch0 <= '9')) {
                            error(Lexer.message("strict.no.octal"), TokenType.STRING, position, limit);
                        }
                    }
                    reset(afterSlash);
                    // Octal sequence.
                ...
                case 'x': {
                    // Hex sequence.
                    final int ch = hexSequence(2, TokenType.STRING);
                ...
                case 'u': {
                    // Unicode sequence.
                    final int ch = hexSequence(4, TokenType.STRING);
    ...
    protected void scanString(final boolean add) {
        ...
        if (ch0 == quote) {
            ...
            error(Lexer.message("missing.close.quote"), TokenType.STRING, position, limit);
        }
    ...
    private void scanTemplate() {
        ...
        error(Lexer.message("missing.close.quote"), TokenType.TEMPLATE, position, limit);
    }
    ...
    private static class EditStringLexer extends Lexer {
        ...
        EditStringLexer(final Lexer lexer, final TokenType stringType, final io.sqreen.nashorn.internal.parser.Lexer.State stringState) {
        ...
        public void lexify() {
            ...
            while (true) {
                ...
                if (ch0 == '\\' && stringType == TokenType.ESCSTRING) {
                    skip(2);
                ...
                if (ch0 == '$' && ch1 == '{') {
                    ...
                    final io.sqreen.nashorn.internal.parser.Lexer.State expressionState = saveState();
                    ...
                    if (braceCount != 0) {
                        error(message("edit.string.missing.brace"), TokenType.LBRACE, expressionState.position - 1, 1);
                    }
                    ...
                    add(TokenType.RPAREN, position - 1, position);
        ...
        while (!stream.isFull() || nested) {
            ...
            if (ch0 == '.' && convertDigit(ch1, 10) != -1) {
                ...
                if (canStartLiteral(type)) {
                    break;
                } else if (type == TokenType.LBRACE && pauseOnNextLeftBrace) {
                ...
                skip(1);
                add(TokenType.ERROR, position - 1);
            }
        }
    }
