...
public abstract class AbstractParser {
    ...
    protected final TokenType next() {
        do {
            nextOrEOL();
        } while (type == TokenType.EOL || type == TokenType.COMMENT);
    ...
    protected final TokenType nextOrEOL() {
        do {
            nextToken();
            if (type == TokenType.DIRECTIVE_COMMENT) {
                checkDirectiveComment();
            }
        } while (type == TokenType.COMMENT || type == TokenType.DIRECTIVE_COMMENT);
    ...
    protected final IdentNode getIdent() {
        ...
        if (isNonStrictModeIdent()) {
            // Fake out identifier.
            identToken = Token.recast(token, TokenType.IDENT);
            // Get IDENT.
        ...
        // Get IDENT.
        final String ident = (String)expectValue(TokenType.IDENT);
        if (ident == null) {
            return null;
        }
        // Create IDENT node.
    ...
    protected final boolean isIdentifierName() {
        ...
        // Fake out identifier.
        final long identToken = Token.recast(token, TokenType.IDENT);
        // Get IDENT.
        final String ident = (String)getValue(identToken);
    ...
    protected final LiteralNode<?> getLiteral() throws ParserException {
        ...
        if (value == null) {
            ...
            node = LiteralNode.newInstance(literalToken, finish, (Lexer.LexerToken)value);
        } else {
