...
import io.sqreen.nashorn.internal.ir.SplitNode;
...
final class SplitIntoFunctions extends NodeVisitor<BlockLexicalContext> {
    ...
    public Node leaveSplitNode(final SplitNode splitNode) {
        ...
        final CallNode callNode = new CallNode(firstLineNumber, token, finish, new AccessNode(Node.NO_TOKEN, Node.NO_FINISH, fn, "call"),
        ...
        if (jumpCount > 0) {
            ...
            splitStateHandler = new SwitchNode(Node.NO_LINE_NUMBER, token, finish, GetSplitState.INSTANCE, cases, null);
        } else {
        ...
        if (splitState.hasBreak) {
            ...
            splitStateHandler = makeIfStateEquals(firstLineNumber, token, finish, BREAK_STATE,
                    enblockAndVisit(new BreakNode(Node.NO_LINE_NUMBER, token, finish, null)), splitStateHandler);
        }
        ...
        if (hasReturn && jumpCount == 0) {
            ...
            splitStateHandler = makeIfStateEquals(Node.NO_LINE_NUMBER, token, finish, RETURN_STATE,
    ...
    private static void addCase(final List<CaseNode> cases, final int i, final Block body) {
        cases.add(new CaseNode(Node.NO_TOKEN, Node.NO_FINISH, intLiteral(i), body));
    }

    private static LiteralNode<Number> intLiteral(final int i) {
        return LiteralNode.newInstance(Node.NO_TOKEN, Node.NO_FINISH, i);
    }

    private static Block createReturnFromSplit() {
        return new Block(Node.NO_TOKEN, Node.NO_FINISH, createReturnReturn());
    }

    private static ReturnNode createReturnReturn() {
        return new ReturnNode(Node.NO_LINE_NUMBER, Node.NO_TOKEN, Node.NO_FINISH, createReturnIdent());
    }
    ...
    private static IdentNode createIdent(final String name) {
        return new IdentNode(Node.NO_TOKEN, Node.NO_FINISH, name);
    }

    private Block enblockAndVisit(final JumpStatement jump) {
        artificialBlock = true;
        final Block block = (Block)new Block(Node.NO_TOKEN, Node.NO_FINISH, jump).accept(this);
    ...
    private static IfNode makeIfStateEquals(final int lineNumber, final long token, final int finish,
        ...
        return new IfNode(lineNumber, token, finish,
                ...
                pass,
                fail == null ? null : new Block(Node.NO_TOKEN, Node.NO_FINISH, fail));
    }
    ...
    public Node leaveBlock(final Block block) {
        if (!artificialBlock) {
            if (lc.isFunctionBody()) {
                ...
                appendSplitReturn(FALLTHROUGH_STATE, Node.NO_LINE_NUMBER);
                if (getCurrentFunctionState().fn.isProgram()) {
                    ...
                    lc.prependStatement(new ExpressionStatement(Node.NO_LINE_NUMBER, Node.NO_TOKEN, Node.NO_FINISH,
