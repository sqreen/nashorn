...
import static io.sqreen.internal.org.objectweb.asm.Opcodes.ATHROW;
import static io.sqreen.internal.org.objectweb.asm.Opcodes.CHECKCAST;
import static io.sqreen.internal.org.objectweb.asm.Opcodes.DUP2;
import static io.sqreen.internal.org.objectweb.asm.Opcodes.GETFIELD;
import static io.sqreen.internal.org.objectweb.asm.Opcodes.GETSTATIC;
import static io.sqreen.internal.org.objectweb.asm.Opcodes.GOTO;
import static io.sqreen.internal.org.objectweb.asm.Opcodes.H_INVOKESTATIC;
import static io.sqreen.internal.org.objectweb.asm.Opcodes.H_INVOKEINTERFACE;
import static io.sqreen.internal.org.objectweb.asm.Opcodes.IFEQ;
import static io.sqreen.internal.org.objectweb.asm.Opcodes.IFGE;
import static io.sqreen.internal.org.objectweb.asm.Opcodes.IFGT;
import static io.sqreen.internal.org.objectweb.asm.Opcodes.IFLE;
import static io.sqreen.internal.org.objectweb.asm.Opcodes.IFLT;
import static io.sqreen.internal.org.objectweb.asm.Opcodes.IFNE;
import static io.sqreen.internal.org.objectweb.asm.Opcodes.IFNONNULL;
import static io.sqreen.internal.org.objectweb.asm.Opcodes.IFNULL;
import static io.sqreen.internal.org.objectweb.asm.Opcodes.IF_ACMPEQ;
import static io.sqreen.internal.org.objectweb.asm.Opcodes.IF_ACMPNE;
import static io.sqreen.internal.org.objectweb.asm.Opcodes.IF_ICMPEQ;
import static io.sqreen.internal.org.objectweb.asm.Opcodes.IF_ICMPGE;
import static io.sqreen.internal.org.objectweb.asm.Opcodes.IF_ICMPGT;
import static io.sqreen.internal.org.objectweb.asm.Opcodes.IF_ICMPLE;
import static io.sqreen.internal.org.objectweb.asm.Opcodes.IF_ICMPLT;
import static io.sqreen.internal.org.objectweb.asm.Opcodes.IF_ICMPNE;
import static io.sqreen.internal.org.objectweb.asm.Opcodes.INSTANCEOF;
import static io.sqreen.internal.org.objectweb.asm.Opcodes.INVOKEINTERFACE;
import static io.sqreen.internal.org.objectweb.asm.Opcodes.INVOKESPECIAL;
import static io.sqreen.internal.org.objectweb.asm.Opcodes.INVOKESTATIC;
import static io.sqreen.internal.org.objectweb.asm.Opcodes.INVOKEVIRTUAL;
import static io.sqreen.internal.org.objectweb.asm.Opcodes.NEW;
import static io.sqreen.internal.org.objectweb.asm.Opcodes.PUTFIELD;
import static io.sqreen.internal.org.objectweb.asm.Opcodes.PUTSTATIC;
import static io.sqreen.internal.org.objectweb.asm.Opcodes.RETURN;
import static io.sqreen.nashorn.internal.codegen.CompilerConstants.constructorNoLookup;
import static io.sqreen.nashorn.internal.codegen.CompilerConstants.staticField;
import static io.sqreen.nashorn.internal.runtime.linker.NameCodec.EMPTY_NAME;
...
import io.sqreen.nashorn.internal.ir.JoinPredecessor;
...
import io.sqreen.internal.org.objectweb.asm.Handle;
import io.sqreen.internal.org.objectweb.asm.MethodVisitor;
import io.sqreen.nashorn.internal.codegen.types.ArrayType;
import io.sqreen.nashorn.internal.codegen.types.BitwiseType;
import io.sqreen.nashorn.internal.codegen.types.NumericType;
...
import io.sqreen.nashorn.internal.objects.NativeArray;
import io.sqreen.nashorn.internal.runtime.ArgumentSetter;
...
import io.sqreen.nashorn.internal.runtime.linker.Bootstrap;
import io.sqreen.nashorn.internal.runtime.linker.NameCodec;
import io.sqreen.nashorn.internal.runtime.linker.NashornCallSiteDescriptor;
...
public class MethodEmitter {
    ...
    MethodEmitter loadConstants() {
        getStatic(classEmitter.getUnitClassName(), CompilerConstants.CONSTANTS.symbolName(), CompilerConstants.CONSTANTS.descriptor());
    ...
    MethodEmitter loadType(final String className) {
        ...
        method.visitLdcInsn(io.sqreen.internal.org.objectweb.asm.Type.getObjectType(className));
    ...
    MethodEmitter load(final String s) {
        ...
        if (length > LARGE_STRING_THRESHOLD) {
            ...
            dup();
            load(length);
            invoke(CompilerConstants.constructorNoLookup(StringBuilder.class, int.class));
            ...
            invoke(CompilerConstants.virtualCallNoLookup(StringBuilder.class, "toString", String.class));
    ...
    private boolean isThisSlot(final int slot) {
        ...
        final int thisSlot = getCompilerConstantSymbol(CompilerConstants.THIS).getSlot(Type.OBJECT);
    ...
    MethodEmitter loadHandle(final String className, final String methodName, final String descName, final EnumSet<ClassEmitter.Flag> flags) {
        final int flag = ClassEmitter.Flag.getValue(flags);
        debug("load handle ");
    ...
    private static class LocalVariableDef {
        ...
        private final io.sqreen.internal.org.objectweb.asm.Label label;
        // The currently live type of the local variable.
        private final Type type;

        LocalVariableDef(final io.sqreen.internal.org.objectweb.asm.Label label, final Type type) {
            this.label = label;
            this.type = type;
        }
    ...
    private void endLocalValueDef(final Symbol symbol, final LocalVariableDef def, final io.sqreen.internal.org.objectweb.asm.Label label) {
    ...
    void store(final Symbol symbol, final Type type, final boolean onlySymbolLiveValue) {
        ...
        if (symbol.hasSlot()) {
            ...
            if(existingDef == null || existingDef.type != type) {
                final io.sqreen.internal.org.objectweb.asm.Label here = new io.sqreen.internal.org.objectweb.asm.Label();
                if(isLiveType) {
    ...
    static io.sqreen.internal.org.objectweb.asm.Label[] getLabels(final Label... table) {
        final io.sqreen.internal.org.objectweb.asm.Label[] internalLabels = new io.sqreen.internal.org.objectweb.asm.Label[table.length];
    ...
    void dynamicSet(final String name, final int flags, final boolean isIndex) {
        ...
        method.visitInvokeDynamicInsn(NameCodec.encode(name),
                CompilerConstants.methodDescriptor(void.class, Object.class, type.getTypeClass()), LINKERBOOTSTRAP, flags | dynSetOperation(isIndex));
    }
    ...
    void dynamicSetIndex(final int flags) {
        ...
        method.visitInvokeDynamicInsn(EMPTY_NAME,
                CompilerConstants.methodDescriptor(void.class, receiver.getTypeClass(), index.getTypeClass(), value.getTypeClass()),
    ...
    void lineNumber(final int line) {
        if (context.getEnv()._debug_lines) {
            ...
            final io.sqreen.internal.org.objectweb.asm.Label l = new io.sqreen.internal.org.objectweb.asm.Label();
            method.visitLabel(l);
    ...
    private final CompilerConstants.Call PRINT            = CompilerConstants.virtualCallNoLookup(PrintStream.class, "print", void.class, Object.class);
    private final CompilerConstants.Call PRINTLN          = CompilerConstants.virtualCallNoLookup(PrintStream.class, "println", void.class, Object.class);
    private final CompilerConstants.Call PRINT_STACKTRACE = CompilerConstants.virtualCallNoLookup(Throwable.class, "printStackTrace", void.class);
