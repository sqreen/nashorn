...
import static io.sqreen.internal.org.objectweb.asm.Opcodes.H_INVOKESPECIAL;
...
import static io.sqreen.internal.org.objectweb.asm.Opcodes.H_NEWINVOKESPECIAL;
import static io.sqreen.internal.org.objectweb.asm.Opcodes.V1_7;
...
import io.sqreen.nashorn.internal.scripts.JS;
import io.sqreen.internal.org.objectweb.asm.ClassWriter;
import io.sqreen.internal.org.objectweb.asm.MethodVisitor;
import io.sqreen.internal.org.objectweb.asm.util.TraceClassVisitor;
import io.sqreen.nashorn.internal.codegen.types.Type;
import io.sqreen.nashorn.internal.ir.FunctionNode;
import io.sqreen.nashorn.internal.ir.debug.NashornClassReader;
import io.sqreen.nashorn.internal.ir.debug.NashornTextifier;
...
public class ClassEmitter {
    ...
    ClassEmitter(final Context context, final String sourceName, final String unitClassName, final boolean strictMode) {
        ...
        cw.visit(V1_7, ACC_PUBLIC | ACC_SUPER, unitClassName, null, pathName(JS.class.getName()), null);
    ...
    private void defineCommonStatics(final boolean strictMode) {
        ...
        field(EnumSet.of(Flag.PUBLIC, Flag.STATIC, Flag.FINAL), CompilerConstants.STRICT_MODE.symbolName(), boolean.class, strictMode);
    }
    ...
    private void defineCommonUtilities() {
        ...
        if (constantMethodNeeded.contains(String.class)) {
            ...
            final MethodEmitter getStringMethod = method(EnumSet.of(Flag.PRIVATE, Flag.STATIC), CompilerConstants.GET_STRING.symbolName(), String.class, int.class);
            getStringMethod.begin();
            getStringMethod.getStatic(unitClassName, CompilerConstants.CONSTANTS.symbolName(), CompilerConstants.CONSTANTS.descriptor())
                        .load(Type.INT, 0)
                        .arrayload()
        ...
        if (constantMethodNeeded.contains(PropertyMap.class)) {
            ...
            final MethodEmitter getMapMethod = method(EnumSet.of(Flag.PUBLIC, Flag.STATIC), CompilerConstants.GET_MAP.symbolName(), PropertyMap.class, int.class);
            getMapMethod.begin();
            ...
            final MethodEmitter setMapMethod = method(EnumSet.of(Flag.PUBLIC, Flag.STATIC), CompilerConstants.SET_MAP.symbolName(), void.class, int.class, PropertyMap.class);
            setMapMethod.begin();
    ...
    private void defineGetArrayMethod(final Class<?> clazz) {
        ...
        getArrayMethod.getStatic(unitClassName, CompilerConstants.CONSTANTS.symbolName(), CompilerConstants.CONSTANTS.descriptor())
                      .load(Type.INT, 0)
                      .arrayload()
                      .checkcast(clazz)
                      .invoke(CompilerConstants.virtualCallNoLookup(clazz, "clone", Object.class))
                      .checkcast(clazz)
                      ._return();
        getArrayMethod.end();
    }
    ...
    MethodEmitter clinit() {
        clinitCount++;
        return method(EnumSet.of(Flag.STATIC), CompilerConstants.CLINIT.symbolName(), void.class);
    }
    ...
    MethodEmitter init() {
        initCount++;
        return method(CompilerConstants.INIT.symbolName(), void.class);
    }
    ...
    final void field(final EnumSet<Flag> fieldFlags, final String fieldName, final Class<?> fieldType, final Object value) {
        fieldCount++;
        cw.visitField(Flag.getValue(fieldFlags), fieldName, CompilerConstants.typeDescriptor(fieldType), null, value).visitEnd();
    }
    ...
    private MethodVisitor methodVisitor(final EnumSet<Flag> flags, final String methodName, final Class<?> rtype, final Class<?>... ptypes) {
        return cw.visitMethod(Flag.getValue(flags), methodName, CompilerConstants.methodDescriptor(rtype, ptypes), null, null);
    }
