...
final class BranchOptimizer {
    ...
    private void branchOptimizer(final BinaryNode binaryNode, final Label label, final boolean state) {
        ...
        case EQ_STRICT:
            ...
            method.conditionalJump(state ? Condition.EQ : Condition.NE, true, label);
            return;

        case NE:
        case NE_STRICT:
            codegen.loadComparisonOperands(binaryNode);
            method.conditionalJump(state ? Condition.NE : Condition.EQ, true, label);
            return;

        case GE:
            codegen.loadComparisonOperands(binaryNode);
            method.conditionalJump(state ? Condition.GE : Condition.LT, false, label);
            return;

        case GT:
            codegen.loadComparisonOperands(binaryNode);
            method.conditionalJump(state ? Condition.GT : Condition.LE, false, label);
            return;

        case LE:
            codegen.loadComparisonOperands(binaryNode);
            method.conditionalJump(state ? Condition.LE : Condition.GT, true, label);
            return;

        case LT:
            codegen.loadComparisonOperands(binaryNode);
            method.conditionalJump(state ? Condition.LT : Condition.GE, true, label);
            return;
