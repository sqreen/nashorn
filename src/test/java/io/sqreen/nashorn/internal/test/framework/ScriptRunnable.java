...
import io.sqreen.nashorn.test.tools.Shell;
...
public final class ScriptRunnable extends AbstractScriptRunnable implements ITest {
    ...
    private void executeInNewProcess() {
        ...
        for (final String str : arguments) {
            if (str.startsWith("--patch-module") ||
                    str.startsWith("--module-path") ||
                    str.startsWith("--add-exports") ||
                    str.startsWith("--add-modules") ||
                    str.startsWith("--add-opens") ||
                    str.startsWith("-Djava.security.") ||
                    str.startsWith("-Dnashorn.jar=") ||
                    str.startsWith("-Dasm.jar=")
            ) {
                cmd.add(str);
            }
        }
...
//        cmd.add("-agentlib:jdwp=transport=dt_socket,server=y,suspend=y,address=0.0.0.0:5005");
        ...
        if (Shell.class.getModule().isNamed()) {
            cmd.add("--module");
            cmd.add(Shell.class.getModule().getName() + "/" + Shell.class.getName());
        } else {
